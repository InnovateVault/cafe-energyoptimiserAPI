...
public class IngestionService {
    ...
    public UploadResponse processCSV(MultipartFile file) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            {
                throw new IllegalArgumentException("CSV header does not match expected columns. Expected order: name,location,timestamp,kwh. Found: " + header);
    ...
    private Row parseRow(String csvLine) {
        ...
        if (parts.length < 4) {
            throw new IllegalArgumentException("Invalid CSV row: " + csvLine);
        }
